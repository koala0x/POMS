...
class OllamaClient:
    ...
    def chat(self, prompt: str) -> str:
        ...
        重试策略:
        - 网络/解析类错误:按 retry_times 重试
        - 超时(ReadTimeout):**不重试**,直接抛错
          原因:Ollama 后端在客户端超时后并不会立即停止生成,
          重试只会让请求堆积、CPU 抢占更严重,失败率反而上升
        """
        ...
        for attempt in range(1, self.retry_times + 1):
            try:
                ...
                # 超时不重试:Ollama 后端可能仍在生成,重试会堆积请求把 CPU 挤爆
                logger.error(
                    "Ollama 推理超时({}s),不重试。建议:减小 batch_size 或 num_predict",
