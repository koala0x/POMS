...
## <a id="q14"></a>Q14：告警推送的决策逻辑是什么？[首次] [重新触发] [升级] 这些标签什么意思？

**短答**：

每 15 分钟 AlertTriggerService 读最新榜单,逐条过"3 道门槛 + 黑名单 → 7 路径决策树"。
门槛拦住的不告警;通过门槛的按决策树 7 条路径(按优先级)判断"发不发 + 发什么标签"。

**详细答**：

### Q14.1 整体流程(每 15 分钟一轮)

```
HotnessService 写完最新榜单(hotness_snapshots)
        ↓
AlertTriggerService.run_once() 被 worker 调用
        ↓
读 hotness_snapshots 最新 window_end 的 Top-100 条记录
        ↓
逐条过 3 道门槛 + 黑名单
        ↓
通过的进入"智能冷却决策树"
        ↓
决策树说"发" → 推 Telegram(带标签)
决策树说"不发" → 静默(日志记 "alert skipped")
```

### Q14.2 第一关:3 道门槛 + 黑名单

每条 hotness 记录必须**同时满足**以下 4 个条件才有资格进入决策树:

```
growth_rate   >= alert_*_growth_threshold    (增长倍数够高)
count_short   >= alert_*_min_count_short     (至少被提到 N 次)
cross_source  >= alert_*_min_cross_source    (至少 N 个源在聊)
entity 不在 alert_exclude_entities 里        (不是被屏蔽的大币)
```

**任一不满足 → 直接跳过,不进决策树,不告警。**

这就是为什么 `alert_exclude_entities` 加了 BTC/ETH 后它们就再也不会推送——在第一关就被拦了,根本走不到决策树。

### Q14.3 第二关:智能冷却决策树(7 条路径,按优先级)

通过门槛后,系统查"这个 entity 上次告警是什么时候、当时 growth 多少、cross_source 多少",然后**按以下顺序逐条判断,命中第一条就停**:

| 优先级 | 条件 | 推送标签 | 含义 |
|---|---|---|---|
| 1 | 从没告警过 | `[首次]` | 新热点出现了 |
| 2 | 距上次告警 ≥ 6 小时 | `[持续 Nh]` | 热了 N 小时还没消停 |
| 3 | growth ≥ 上次 × 1.5 | `[升级 → growth ×2.0]` | 比上次又涨了一大截,在加速 |
| 4 | cross_source 比上次多 | `[跨源升级 +1]` | 从小圈子扩散到大圈子了 |
| 5 | growth 比上次涨 ≥ 30% | `[growth +35%]` | 没翻倍但在持续走高 |
| 6 | 距上次 < 60 分钟 + 以上都没命中 | **不推送** | 冷却中,无质变 |
| 7 | 距上次 ≥ 60 分钟 + 仍达标 | `[重新触发]` | 冷却到期,还在热 |

**路径 6 是"不刷屏"的核心**:同一个 entity 在 60 分钟内如果没有"质变"(翻倍/跨源/涨 30%),就不会再推。

**路径 7 是"不漏信号"的兜底**:冷却到期后如果它还在榜上,再提醒一次。
...
```
门槛检查:
  growth 8.5 >= 1.5(1h 阈值）  ✅
  count  42  >= 2               ✅
  cross  2   >= 1               ✅
  EIGEN 在 alert_exclude_entities 里?  ❌ 不在,放行

决策树:
  路径 1: EIGEN 之前告警过吗? → 告警过(不是首次)
  路径 2: 距上次告警 ≥ 6 小时? → 只过了 75 分钟,不命中
  路径 3: growth 8.5 ≥ 上次(7.0) × 1.5 = 10.5? → 8.5 < 10.5,不命中
  路径 4: cross_source 2 > 上次(2)? → 没增加,不命中
  路径 5: growth 涨幅 = 8.5/7.0 - 1 = 21% ≥ 30%? → 21% < 30%,不命中
  路径 6: 距上次 < 60 分钟? → 75 分钟 > 60 分钟,不命中
  路径 7: 距上次 ≥ 60 分钟 + 仍达标 → ✅ 命中!
...
| 标签 | 你该怎么理解 | 行动建议 |
|---|---|---|
| `[首次]` | "新东西冒出来了" | 重点关注,可能是早期信号 |
| `[持续 6h]` | "热了半天还没消" | 大概率是真趋势,不是一阵风 |
| `[升级 → growth ×2.0]` | "在加速爆发" | 高优先级,可能要做决策 |
| `[跨源升级 +1]` | "从小圈子扩散了" | 信号可信度提升 |
| `[growth +35%]` | "温和走高" | 持续关注,还没到爆发 |
| `[重新触发]` | "老热点还没退" | 低优先级,除非你在跟踪它 |

### Q14.6 你能调的参数(影响决策树行为)

| 参数 | 默认 | 调大效果 | 调小效果 |
|---|---|---|---|
| `alert_cooldown_minutes` | 60 | 冷却更久,同 entity 告警更稀 | 冷却更短,告警更密 |
| `alert_heartbeat_hours` | 6 | 心跳间隔更长,`[持续]` 更少 | 心跳更频繁 |
| `alert_escalation_growth_multiplier` | 1.5 | `[升级]` 更难触发(要翻更多倍) | 更容易触发 |
| `alert_growth_delta_pct` | 0.3 | `[growth +X%]` 更严(涨 50% 才升级) | 更松(涨 20% 就升级) |

大多数情况下这些不用动。如果觉得 `[重新触发]` 太频繁,把 `alert_cooldown_minutes` 从 60 调到 120(2 小时冷却)。

### Q14.7 实时通道(RealtimeAlertService)的区别

除了整点告警,还有一条"实时通道"——EntityExtractor 每写入 50 条新提及就触发一次轻量计算。它的决策树**完全一样**(共用同一个 `decide_alert` 函数),区别是:

- 标签前缀带 `[实时]`(如 `[实时][首次]`)
- 阈值更严(`realtime_growth_threshold=30`,比整点高很多)
- 与整点通道**共享冷却 dict**:实时推了,整点就不会重复推
